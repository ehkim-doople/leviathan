...
#define	BUF_UNIT_LEN	256

char add_string(struct dynBuf *stBuf, char *str)
{
	if (!stBuf || !str)
		return 1;

	if (stBuf->buf == NULL) {
		stBuf->buf = (char *)malloc(BUF_UNIT_LEN);
		if (stBuf->buf == NULL)
			return 1;

		stBuf->buf[0] = 0;
		stBuf->blen = BUF_UNIT_LEN;
	}

	while (strlen(stBuf->buf) + strlen(str) > stBuf->blen) 
	{
		stBuf->buf = (char *)realloc(stBuf->buf, stBuf->blen + BUF_UNIT_LEN);
...
char *hash_string(struct hash *h)
{
	struct nlist *np = NULL, *op = NULL;
	int    i;
	struct dynBuf	buf = { 0, 0 };
	char	res = 0;

	for (i=0;i<h->size && !res;i++) 
	{
		if (h->hashtab[i]) 
		{
			np = h->hashtab[i];
			while (np != NULL && !res) 
			{
				op = np->next;
				if (np->key) {
					res |= add_string(&buf, np->key);
					res |= add_string(&buf, "=");
				}

				if (np->value) {
					res |= add_string(&buf, np->value);
					res |= add_string(&buf, "\n");
				}
				np = op;
			}
