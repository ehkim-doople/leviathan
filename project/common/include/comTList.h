...
public:
	...
	bool isObj(T*p);
	int getObjIdx(T*p);
	bool setObj(int idx, T* p);
	bool setObjChange(int idx, T* p);
	void clear();
...
template <typename T>
int CMemList<T>::getObjIdx(T* Obj)
{
	int i;
	for (i = 0; i < m_nMaxCount; i++)
	{
...
// sequential add
template <typename T>
bool CMemList<T>::setObj(int idx, T* p) {
...
template <typename T>
bool CMemList<T>::setObjChange(int idx, T* p) {
	if (!ISINCLUDE(idx, m_nMaxCount)) return false;
	if (m_nFlagArray[idx].getCount() == 2) {
		m_pArray[idx] = p;
		return true;
