 ...
 */
node RBTree::new_node(void* k, void* v, node newNode, color n_color, node left, node right)
{
    node result = newNode;
    result->key = k;
    result->value = v;
 ...
 */
void RBTree::rbtree_insert(rbtree t, node inserted_node, compare_func compare)
{
    ...
    else
    {
        ...
        while (1)
        {
            int comp_result = compare(inserted_node->key, n->key);
            if (comp_result == 0)
            {
