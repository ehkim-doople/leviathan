...
CMetaData::~CMetaData()
{
	...
	HMODULE hModule = GetModuleHandleA("InfiniA-Library.dll");
	FreeLibrary(hModule);
}
...
bool CMetaData::init()
{
	...
	if (!gs_pCore->initDefaultPool(256, 4096, 0)) {
		comErrorPrint(_T("g_cCore.initDefaultPool(256, 1024, 0) has Failed!"));
		return false;
	}

	m_pParamList = new (std::nothrow) CSList();
	if (!m_pParamList) {
	...
	m_pEventList = new (std::nothrow) CSList();
	if (!m_pEventList) {
		printf("init() new m_pParamList error\n"); return false;
		//gs_cLogger.DebugLog(LEVEL_ERROR, _T("new m_pParamList")); 
	}
	...
	if (!m_pParamPool->alloc(4096, eAlloc_Type_alloc)) {
		delete m_pParamPool;
		m_pParamPool = NULL;
		printf("init() alloc m_pEventPool error\n"); return false;
	}
...
void CMetaData::dataParsingNSet() 
{
	...
	// parameter parsing
	pList = (CStringList *)m_pParamList->getNext(&i);
	while (pList) {
	...
	// event parsing
	i = 0;
	pList = (CStringList *)m_pEventList->getNext(&i);
	while (pList) {
