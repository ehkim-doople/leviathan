...
CHandler::CHandler()
{
	...
	memset(m_pAnalysorList, 0, sizeof(m_pAnalysorList));
}
...
CHandler::~CHandler()
{
	...
	{
		if (m_pAnalysorList[i]) {
			delete m_pAnalysorList[i];
			m_pAnalysorList[i] = NULL;
		}
	}
}
...
bool CHandler::initAnalistConfig()
{
	...
	for (i = 0; i < eAType_Max; i++)
	{
		...
		cAnalysor.initConfig(g_pProcessConfig, szSector);
		if (cAnalysor.isAvailable()) {
			switch (i) {
			case eAType_KeyValue		: m_pAnalysorList[i] = new CAnalysor_KeyValue();		break;
			case eAType_ExtractNToFile	: m_pAnalysorList[i] = new CAnalysor_ExtractNToFile();  break;
			case eAType_DateTime		: m_pAnalysorList[i] = new CAnalysor_DateTime();		break;
			case eAType_BetweenLines	: m_pAnalysorList[i] = new CAnalysor_BetweenLines();	break;
			case eAType_Average			: m_pAnalysorList[i] = new CAnalysor_Average();			break;
			}
			bRes = m_pAnalysorList[i]->initConfig(g_pProcessConfig, szSector);
			if (!bRes) {
				sprintf(g_szMessage, "Analysor->initConfig has Failed! g_pProcessConfig[%s] szSector[%s]", g_pProcessConfig, szSector);
				comErrorPrint(g_szMessage);
				return false;
			}
		}
...
bool CHandler::parsingLine(char *pLine)
{
	...
	{
		if (m_pAnalysorList[i]) {
			if (!m_pAnalysorList[i]->parsingLine(pLine)) {
				sprintf(g_szMessage, "Analysor Index:%d\n", i);
				comErrorPrint(g_szMessage);
				return false;
			}
		}
...
void CHandler::Report(CLogger *pLogger)
{
	...
	{
		if (m_pAnalysorList[i]) {
			m_pAnalysorList[i]->report(pLogger);
		}
	}
}
