...
CAnalyzer_BetweenLines::CAnalyzer_BetweenLines()
{
	m_pClassList = NULL;
}
...
bool CAnalyzer_BetweenLines::initConfig(char *pConfigFile, char *pSector)
{
	int nRes, nCount, i, j;
...
bool CAnalyzer_BetweenLines::parsingLine(char *pLine)
{
	int i=0;
	STBetween_Class *pClass;
	if (m_pClassList) {
...
void CAnalyzer_BetweenLines::parsingClass(char *pLine, STBetween_Class *pClass)
{
	char *pPos;
	...
	if (pPos) { // LINE_0
		...
		if (pPos) {
			...
			pInterval->nInterval[pInterval->nIntervalIdx++] = getMiliseconds(&stRes, g_stConfig.pDTE->nLastLen) * 1.0f;
...
void CAnalyzer_BetweenLines::report(CLogger *pLogger)
{
