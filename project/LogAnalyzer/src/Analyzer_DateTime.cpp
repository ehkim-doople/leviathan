...
CAnalyzer_DateTime *g_pAnalyzer_DateTime;


CAnalyzer_DateTime::CAnalyzer_DateTime()
{
	m_nCurSlot = 0;
	m_nListIdx = 0;
	m_pTimeSlot = NULL;
...
bool CAnalyzer_DateTime::initConfig(char *pConfigFile, char *pSector)
{
	char szBuf[64];
	int nRes;
...
bool CAnalyzer_DateTime::initTimeSlot(STMTime ***pSlot)
{
...
bool CAnalyzer_DateTime::parsingLine(char *pLine)
{
	STDTime stTime;
...
void CAnalyzer_DateTime::addDataToTimeSlot(STDTime *pTime)
{
...
void CAnalyzer_DateTime::addDataToTimeSlot2(STDTime *pTime)
{
	// 1. 현재 Idx 를 기준으로 만 서칭
	// TODO :  시간이 안맞으면, 앞, 뒤 서칭 추가
...
void CAnalyzer_DateTime::calculate(STMTime *pMTime)
{
...
void CAnalyzer_DateTime::reportDetail(CLogger *pLogger, STMTime *pMTime)
{
	int nSize = 600 << 5;
...
void CAnalyzer_DateTime::report(CLogger *pLogger)
{
	int nPos = 0;
	int i = 0, j = 0, nIdx;
