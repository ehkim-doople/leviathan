...
static const int g_nSizeList[IDX_BUF_MAX] = { 1,2,4,8,16,32,64,128,256,512,1024,2048,4096,8192,16348,32768,65536,131072,262144,524288,1048576,2097152 };
	...
	for(j = 0; j < m_nAllocCount; j++)
    {
		...
		if(!m_nFlagArray[i].getCount()) 
		{
			if(!m_nFlagArray[i].atomic_compare_exchange(1,0)) 
			{
				...
				m_nLastCount=i;
				memset(&m_pChunk[i << m_nShift], 0, g_nSizeList[m_nShift]);
				return &m_pChunk[i << m_nShift];
			}
		}
		i++;
    }    
