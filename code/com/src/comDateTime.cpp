...
int CFileTime::setFileTime(TCHAR *pFilePath)
{
	...
	//_stprintf(szDate, "%04d%02d%02d", localTime.wYear, localTime.wMonth, localTime.wDay);
	//m_nDate = atoi(szDate);
	//_stprintf(szDate, "%02d%02d%02d", localTime.wHour, localTime.wMinute, localTime.wSecond);
	//m_nTime = atoi(szDate);

	_stprintf(szDate, "%04d%02d%02d", sysTm.wYear, sysTm.wMonth, sysTm.wDay);
	m_nDate = atoi(szDate);
	_stprintf(szDate, "%02d%02d%02d", sysTm.wHour, sysTm.wMinute, sysTm.wSecond);
	m_nTime = atoi(szDate);
...
bool setLocalFileTime(time_t *fileTime, int *pDate, int *pTime)
{
	...
	_stprintf(szDate, "%04d%02d%02d", tmTime.tm_year, tmTime.tm_mon, tmTime.tm_mday);
	*pDate = atoi(szDate);
	_stprintf(szDate, "%02d%02d%02d", tmTime.tm_hour, tmTime.tm_min, tmTime.tm_sec);
	*pTime = atoi(szDate);
...
{
	struct	tm *gm_timeInfo, sMtm;
	TCHAR szDate[16];
#ifndef WIN32
	gm_timeInfo = gmtime_r(&fileTime, gm_timeInfo);
#else
	gm_timeInfo = gmtime(fileTime);
	memcpy(&sMtm, gm_timeInfo, sizeof(sMtm));
#endif
	sMtm.tm_year += 1900;
	sMtm.tm_mon += 1;

	_stprintf(szDate, "%04d%02d%02d", sMtm.tm_year, sMtm.tm_mon, sMtm.tm_mday);
	m_nDate = atoi(szDate);
	_stprintf(szDate, "%02d%02d%02d", sMtm.tm_hour, sMtm.tm_min, sMtm.tm_sec);
	m_nTime = atoi(szDate);
...
bool CFileTime::isCompare(CFileTime *pCompare, E_OPERATOR eOP)
{
	switch (eOP)
	{
	case eOperator_EQ: if (pCompare->getDate() == m_nDate && pCompare->getTime() == m_nTime) return true; return false;
	case eOperator_NE: if (pCompare->getDate() != m_nDate && pCompare->getTime() != m_nTime) return true; return false;
	case eOperator_GT:
		if (pCompare->getDate() < m_nDate) return true;
		if (pCompare->getDate() > m_nDate) return false;
		if (pCompare->getTime() < m_nTime) return true; return false;
	case eOperator_LT:
		if (pCompare->getDate() > m_nDate) return true;
		if (pCompare->getDate() < m_nDate) return false;
		if (pCompare->getTime() > m_nTime) return true; return false;
...
//#####################################################################
void getCurrentTime(STTime *pTime)
{
#ifndef WIN32
	struct	tm	ltm;
	ltm = &_tm;
...
void getTimeInterval(STTime *pTime1, STTime *pTime2, STTime *pTimeTarget, int nLastLen)
{
	int nValue = 0, i;
	if (pTime2->nLast < pTime1->nLast) {
		nValue = 1;
		for (i = 0; i < nLastLen; i++)
		{
			nValue = nValue * 10;
		}
