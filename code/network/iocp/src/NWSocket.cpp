...
{
	if (m_socket != INVALID_SOCKET) {
		gs_cLogger.DebugLog(LEVEL_ERROR, _T("m_socket already create! nFD:%ld nSIdx:%d msg%s"), m_socket, m_nSIdx, err_desc_socket[CERROR_ALREADY_SOCKET]);
		closeSocket();
...
int CNWSocket::SockServerInit(const int nPort, const char* szIP)
{
	...
	if (bind(m_socket, (LPSOCKADDR)&saServer, sizeof(sockaddr)) < 0) {
			gs_cLogger.DebugLog(LEVEL_ERROR, _T("nPort[%d] szIP[%s] nFD[%lu] nSIdx:[%d] WSAError[%d] err_desc[%s]"),
				nPort, szIP, m_socket, m_nSIdx, WSAGetLastError(), err_desc_socket[CERROR_BIND]);
		// 에러 처리
		return CERROR_BIND;
	}
	...
	if (listen(m_socket, 5) == -1) {
		gs_cLogger.DebugLog(LEVEL_ERROR, _T("nPort[%d] szIP[%s] nFD[%lu] nSIdx:[%d] WSAError[%d] err_desc[%s]"), nPort, szIP, m_socket, m_nSIdx, WSAGetLastError(), err_desc_socket[CERROR_LISTEN]);
		// 에러 처리
...
int CNWSocket::bindAccept(SOCKET serverSock)
{
		...
		&m_lpRecvData->nCurLen, reinterpret_cast<LPOVERLAPPED>(m_lpRecvData)))
	{
		...
		{
			gs_cLogger.DebugLog(LEVEL_ERROR, _T("nFD[%lu] nSIdx:[%d] serverSockFD[%d] WSAError[%d] err_desc[%s]"), m_socket, m_nSIdx, serverSock, WSAGetLastError(), err_desc_socket[CERROR_BINDACCEPT]);
...
int CNWSocket::doAccept(CRTSQueue *pRTSQueue)
{
	...
	if(0 != nRemoteSockaddrLen) {
		...
		gs_cLogger.DebugLog(LEVEL_ERROR, _T("nFD[%lu] nSIdx:[%d] WSAError[%d] err_desc[%s]"), m_socket, m_nSIdx, WSAGetLastError(), err_desc_socket[CERROR_ACCEPT]);
	...
	*/
	if(pRTSQueue->registerSocket((HANDLE)m_socket, this) == CERROR_REGIOCP) {
		gs_cLogger.DebugLog(LEVEL_ERROR, _T("registerSocket has Failed! nFD[%lu] nSIdx:[%d] err_desc[%s]"), m_socket, m_nSIdx);
...
int CNWSocket::RecvPacket(int nRecvSize)
{
		...
		&nFlags, reinterpret_cast<LPWSAOVERLAPPED>(m_lpRecvData), NULL) == SOCKET_ERROR) 
	{
		...
		if(nRet != WSA_IO_PENDING && nRet != WSAEWOULDBLOCK)  { // 에러발생
			gs_cLogger.DebugLog(LEVEL_ERROR, _T("nFD[%lu] nSIdx:[%d] WSAGetLastError[%d] err_desc[%s]"), m_socket, m_nSIdx, nRet, err_desc_socket[CERROR_SENDRECV]);
...
int CNWSocket::Send(char *pMsg, int nLen, LPSOCKETDATA lpSendData) // nLen : msg Len
{
	...
	if (nRes == SOCKET_ERROR) {
		...
		if (errorCode == WSA_IO_PENDING || errorCode == WSAEWOULDBLOCK) {
			gs_cLogger.DebugLog(LEVEL_WARN, "nFD[%lu] nSIdx:[%d] WSA_IO_PENDING nTotLen[%d]", m_socket, m_nSIdx, sWsaBuf.len);
		}
		else { // 에러 발생
			m_lpSendData = lpSendData;
			g_pErrIocpQueue->putRTSQueue(OP_CLOSE, this);
			const char *pDesc = err_desc_socket[CERROR_SENDRECV];
			gs_cLogger.DebugLog(LEVEL_ERROR, _T("nFD[%lu] nSIdx:[%d] curLen[%d] totLen[%d] errorCode[%d] err_desc[%s]"), m_socket, m_nSIdx, lpSendData->nCurLen, sWsaBuf.len, errorCode, pDesc);
			return CERROR_SENDRECV;
		}
	}
...
//		setsockopt(m_socket, SOL_SOCKET, SO_LINGER | SO_REUSEADDR, (char *)&li, sizeof(li));
		...
		clear();
		if(bLog) gs_cLogger.PutLogQueue(LEVEL_INFO, "nSIdx[%d] nFD[%lu] socket Close complete", m_nSIdx, nFD);
		return 1;
	}	
...
bool CNWSocket::ConnectTo(const char* szIP, int nPort, int miliseconds, CRTSQueue *pRTSQueue)
{
	...
	if (nRet != NO_ERROR) {
		_stprintf(g_szMessage, _T("ioctlsocket Fail nRet[%d] nSIdx[%d] FD[%lu] [%s:%d]"), nRet, m_nSIdx, m_socket, szIP, nPort);
	...
	if(nRet == -1) {
		if (WSAGetLastError() != WSAEWOULDBLOCK) {
			_stprintf(g_szMessage, _T("connect has failed! nSIdx[%d] FD[%lu] [%s:%d]"), m_nSIdx, m_socket, szIP, nPort);
			goto CONNECTTO_ERROR;
		}
	}
	...
	if (FD_ISSET(m_socket, &wrset))
	{
		...
		if (bindRecv() == CERROR_NONE) {
			gs_cLogger.PutLogQueue(LEVEL_INFO, "nSIdx[%d] FD[%lu] IP[%s] nPort[%d] ConnectTo Complete", m_nSIdx, m_socket, szIP, nPort);
			DELSOCKETSTATUS(SOCK_STATUS_CONNECTED_ERROR);
			return true;
		}
	}
	else {
		_stprintf(g_szMessage, _T("select Fail nRet[%d] nSIdx[%d] FD[%lu] [%s:%d]"), nRet, m_nSIdx, m_socket, szIP, nPort);
	}
...
bool CNWSocket::setOption(int level, int optname, const void *optval, int optlen)
{
	...
	if (iResult == SOCKET_ERROR) {
		_stprintf(g_szMessage, _T("iResult[%d] nSIdx[%d] FD[%lu] error[%d]"), iResult, m_nSIdx, m_socket, WSAGetLastError());
