...
void CNWCommunicator::comproc_sendComplete(STComplete *pComplete)
{
		...
		if(!pComplete->bSuccess || pComplete->bytesTrans == 0)
	{
		if(pSocket->Connected())
		{
			...
			gs_cLogger.DebugLog(LEVEL_ERROR, "nSIdx[%d] nFD[%d] WSAGetLastError[%d] bSuccess[%d] nBytesTrans[%d]", pSocket->GETSESSIONIDX(), pSocket->GETSOCKET(),
...
void CNWCommunicator::procAtonceClose(CNWSocket *lpCompletion, StDisconInfo sInfo)
{
	...
	int nFD = lpCompletion->GETSOCKET();

	if (lpCompletion->ISSOCKETSTATUS(SOCK_STATUS_ACCEPTED)) {
		if (m_pConnectList->del(lpCompletion)) {
			gs_cLogger.PutLogQueue(LEVEL_TRACE, "FD[%lu] delSession nSIdx[%d] ListCount[%d]", nFD, lpCompletion->GETSESSIONIDX(), m_pConnectList->size());
		}
		else {
			gs_cLogger.DebugLog(LEVEL_ERROR, "FD[%lu] delSession nSIdx[%d] ListCount[%d]", nFD, lpCompletion->GETSESSIONIDX(), m_pConnectList->size());
		}
	}
...
int CNWCommunicator::timeoutProcess()
{
	...
	while ((pSock = m_pConnectList->getNext(&nCurIdx)))	{
		if (GETTICKCOUNT() - pSock->GETTIMETICK() > m_nTimeout)	{
			...
			nCount++;
			nLen += _stprintf(szBuf + nLen, " %d:%u ", pSock->GETSESSIONIDX(), pSock->GETSOCKET());
		}
		nCurIdx++;
	}
