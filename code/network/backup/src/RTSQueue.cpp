...
int CRTSQueue::init(int nMax, int nThreadCnt)
{
	...
	else return CERROR_ALREADY_ALLOC;
...
int CRTSQueue::putRTSQueue(int nOPCode, void *pCompletionKey)
{
	...
	if(lpOverlapped == NULL) {
		//gs_cLogger.PutLogQueue(LEVEL_ERROR, "empty from stack - TYPE:LPOVERLAPPED_EX nTot[%d] nCur[%d]", m_pEventMemMgr->GETMAXCOUNT(), m_pEventMemMgr->GETUSECOUNT());
